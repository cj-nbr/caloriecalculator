...
// 1. Add ALL_CALCULATORS to imports
...
// 2. Add dynamic selection of tools in the JS frontmatter
if (!content.includes('const relatedTools = ALL_CALCULATORS')) {
  content = content.replace(
    '---',
    `
...
---`
  ...
  // Actually wait, there are two `---` blocks. We need to insert it before the LAST `---`.
  // Let's do it with regex.
}

fs.writeFileSync('scripts/fix_content_layout.cjs', `
const fs = require('fs');
let content = fs.readFileSync('src/layouts/ContentLayout.astro', 'utf8');
