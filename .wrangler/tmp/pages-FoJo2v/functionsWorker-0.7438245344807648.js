...
__name(pickNutrients, "pickNutrients");
...
async function onRequestGet(context) {
  ...
  const json = /* @__PURE__ */ __name((obj, status = 200, maxAge = 86400) => new Response(JSON.stringify(obj), {
    status,
    headers: {
  ...
  const endpoint = `https://api.nal.usda.gov/fdc/v1/foods/search?api_key=${apiKey}&query=${encodeURIComponent(query)}&pageSize=10&dataType=Foundation,SR%20Legacy,Branded`;
  try {
    const res = await fetch(endpoint, { headers: { Accept: "application/json" } });
    if (!res.ok) {
      const status = res.status === 429 ? 429 : 502;
      const msg = res.status === 429 ? "The nutrition service is busy right now. Please try again in a moment." : "We couldn't reach the nutrition database. Please try again shortly.";
      return json({ error: msg, results: [] }, status, 30);
    }
    const data = await res.json();
    const foods = Array.isArray(data.foods) ? data.foods : [];
    const rank = { Foundation: 0, "SR Legacy": 1, "Survey (FNDDS)": 2, Branded: 3 };
    const results = foods.map(normalize).filter((f) => f.per100g.calories != null || f.per100g.protein != null).sort((a, b) => (rank[a.dataType] ?? 5) - (rank[b.dataType] ?? 5)).slice(0, 8);
    if (results.length === 0) {
      return json(
...
// ../.wrangler/tmp/pages-FoJo2v/functionsRoutes-0.14458095396583903.mjs
var routes = [
  {
